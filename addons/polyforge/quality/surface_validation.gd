...
static func evaluate(parts: Array, required := false, require_extended := false) -> Dictionary:
	...
	for part in parts:
		...
		# Construction proportions belong to the authored mesh frame. World AABBs
		# change when a valid plate or beam is rotated and are unsuitable here.
		var local_bounds: AABB = part.mesh.get_aabb()
		var axes := _axes(local_bounds.size)
		axes.sort()
