...
func _horsehead(size: float, frame_depth: float, sand: Material,
	...
	for index in range(3):
		plate.add("insert_%02d" % index, insert_mesh,
			...
			_options("plate", "decorative", ["horsehead", "arcane"], {
				"surface_options": {"maximum_thickness_ratio": 0.30}}))
...
func build(p) -> Dictionary:
	...
	_add(asset, "exhaust_stack", _part(Stock.cylinder(size * 0.055, size * 0.075,
		...
		["machine", "exhaust"], {"surface_options": {"axis": "y"}})
	...
	return {
		...
		"readability": {
			...
			"critical_parts": {
				...
				"crank_disk": {"minimum_visible_fraction": 0.10,
					"views": [0.0, 45.0, 315.0]},
				"rear_flywheel": {"minimum_visible_fraction": 0.10,
